...
async function finding(ruleId: string, severity: Severity, filePath: string, message: string, options: Partial<Pick<Finding, "structuralPath" | "confidence" | "evidence" | "suggestedAction">> = {}): Promise<Finding> {
...
export async function runDoctor(inventory: InventoryResult): Promise<Finding[]> {
  ...
  for (const base of inventory.bases) {
    ...
    for (const reference of base.references.filter((item) => item.confidence === "exact" && !properties.has(item.propertyName))) {
      findings.push(await finding("MISSING_PROPERTY", "warning", base.snapshot.path, `Property '${reference.propertyName}' has no instances in the Vault.`, { structuralPath: reference.structuralPath, evidence: reference.evidence, suggestedAction: "create-refactor" }));
    }
  ...
  for (const group of caseGroups.values()) if (group.length > 1) findings.push(await finding("CASE_DRIFT", "warning", "", `Properties differ only by case: ${group.join(", ")}.`, { evidence: group.join(", "), suggestedAction: "create-refactor" }));
